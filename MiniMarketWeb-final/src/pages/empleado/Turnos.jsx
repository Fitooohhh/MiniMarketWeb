...
export default function Turnos() {
  ...
  const cargarSolicitudesIntercambio = async () => {
    try {
      const idEmpleado = user.profile.id_empleado
      const data = await turnosService.obtenerSolicitudesIntercambio(idEmpleado, 'solicitado')
      setSolicitudesIntercambio(data)
    } catch (error) {
      console.error('Error al cargar solicitudes de intercambio:', error)
    }
  }

  const handleSolicitarIntercambio = (idTurno) => {
    // Aquí iría la lógica para solicitar intercambio
    toast.info('Función de intercambio en desarrollo')
  }

  const handleAceptarIntercambio = async (idIntercambio) => {
    try {
      await turnosService.aceptarIntercambio(idIntercambio)
      toast.success('Intercambio aceptado correctamente')
      cargarTurnos()
      cargarSolicitudesIntercambio()
    } catch (error) {
      toast.error('Error al aceptar intercambio')
      console.error(error)
    }
  }

  const handleRechazarIntercambio = async (idIntercambio) => {
    try {
      await turnosService.rechazarIntercambio(idIntercambio, 'No disponible')
  ...
  const getEstadoIcon = (estado) => {
    switch (estado) {
      ...
      case 'cumplido': return <Users className="w-4 h-4" />
      case 'ausente': return <AlertCircle className="w-4 h-4" />
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Gestiona tu horario y solicita intercambios</p>
        </div>
      ...
      {solicitudesIntercambio.length > 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <h3 className="text-lg font-medium text-yellow-900 mb-3">Solicitudes de Intercambio</h3>
          <div className="space-y-3">
            {solicitudesIntercambio.map((solicitud) => (
              <div key={solicitud.id_intercambio} className="bg-white rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-gray-900">
                      Intercambio para {new Date(solicitud.fecha_intercambio).toLocaleDateString()}
                    </p>
                    <p className="text-sm text-gray-600">
                      Solicitado por: {solicitud.empleado_solicita.nombre}
                    </p>
                    <p className="text-sm text-gray-600">
                      Turno: {solicitud.turno_original.tipo_turno.nombre} ({solicitud.turno_original.tipo_turno.hora_inicio} - {solicitud.turno_original.tipo_turno.hora_fin})
                    </p>
                    {solicitud.motivo && (
                      <p className="text-sm text-gray-600 mt-1">Motivo: {solicitud.motivo}</p>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleAceptarIntercambio(solicitud.id_intercambio)}
                      className="px-3 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm"
                    >
                      Aceptar
                    </button>
                    <button
                      onClick={() => handleRechazarIntercambio(solicitud.id_intercambio)}
      ...
      <div className="space-y-4">
        {diasMes.length > 0 ? (
                  ...
                  <div key={turno.id_turno} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {turno.estado === 'programado' && (
                        <button
                          onClick={() => handleSolicitarIntercambio(turno.id_turno)}
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">No tienes turnos asignados para este período</p>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <span className="ml-2 font-medium text-red-600">
              {turnos.filter(t => t.estado === 'ausente').length}
            </span>
          </div>
        </div>
      </div>
